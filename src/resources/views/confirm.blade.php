...
<div class="section">
  <div class="section__inner">
    ...
    <form action="/thanks" method="post" class="contact-form contact-form--confirm">
      @csrf
      <dl class="confirm-table">
        <dt class="confirm-table__title">お名前</dt>
        <dd class="confirm-table__content">{{ $contact['first_name'] }}　{{ $contact['last_name'] }}<input type="hidden" name="first_name" value="{{ $contact['first_name'] }}"><input type="hidden" name="last_name" value="{{ $contact['last_name'] }}"></dd>
        ...
        <dd class="confirm-table__content">{{ substr($contact['tel'], 0, 3) }}-{{ substr($contact['tel'], 3, 4) }}-{{ substr($contact['tel'], 7) }}<input type="hidden" name="tel" value="{{ $contact['tel'] }}"></dd>
